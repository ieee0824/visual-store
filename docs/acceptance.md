...
| V03 | Passed | RGB and RGBA sequences round-trip exactly through VP9 and PNG reconstruction, including transparent and partially transparent pixels and nonzero hidden RGB at alpha 0. Checked payload accounting includes the separate alpha packets. |
| V04 | Passed | An odd 257×5 fixture covers every byte value 0–255, one-pixel red/green/blue lines, adjacent values, and a one-pixel frame change without a changed byte after VP9 decode and PNG reconstruction. |
| V05 | Passed | `tests/reconstruction.rs` extracts and canonically serializes reconstruction metadata, then rebuilds RGB/RGBA PNGs for filters 0–4 and split IDAT input. Original IHDR, accepted color/physical/text/private chunks, ordering, IDAT-relative placement, and the existing pixel/scanline/non-IDAT hashes all match. |
